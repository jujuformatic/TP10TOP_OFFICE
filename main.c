...
void lireCommande(FILE *commande, char *nombreCommande)
{
	...
	fscanf(factureFile, "%s", client);
	fprintf(factureFile, "Client : %s", client);
	
	while (fscanf(commande, "%d %d\n", &serieCommande, &quantiteCommande) != NULL)
	{
		/*
		produits = fopen("produit.txt", "r");
		while (fscanf(produits, "%d %s %f", &serieProduit, nomProduit, &prix) != NULL && serieProduit != serieCommande)
		{
			printf("<2>");
		}
		fclose(produits);
*/
		fprintf(factureFile, "%d %s (PU=%f€) :: %f€", quantiteCommande, nomProduit, prix, prix*quantiteCommande);

		total += prix * quantiteCommande;
		
	}

	fprintf(factureFile, "\tTOTAL = %f€", total);
	fclose(factureFile);
